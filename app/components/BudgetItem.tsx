...
const BudgetItem: React.FC<BudgetItemProps> = ({ budget, enableHover }) => {
  ...
  const progressValue =
    transactionAmount > budget.amount
      ? 100
      : (transactionAmount / budget.amount) * 100;
  const hoverClasse =
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="text-xl font-bold text-accent">{budget.amount} Ar</div>
      </div>
      <div className="flex justify-between items-center mt-4 text-gray-500 text-sm">
        <span>{transactionAmount} Ar dépensés</span>
        <span>{remainingAmount} Ar restant</span>
      </div>
      <div className="">
        <progress
